...
function(x, variable, prec=2, absol=TRUE, zmax1=0, zmax2=0) {
  ...
  x11()
  ifelse(is.na(d[1])==TRUE, par(mfrow=c(1,2), mar=c(10,5,5,5)), par(mfrow=c(1,3), mar=c(10,5,5,5)))
  # plot 1 accuracy
  ifelse(absol==TRUE, a<-abs(a1-a2), a<-a1-a2)
  ...
  if(x$FLAG[1]==TRUE) {  graphics::image(x=sort(unique(x$G)), y=sort(unique(x$R)), z=a ,col = gradient ,xlab="Population size monitored",ylab="Study period (# sampling occasions)", xlim=c(min(x$G),max(x$G)), ylim=c(min(x$R),max(x$R)), axes=FALSE)}
  if(x$FLAG[1]==FALSE) {  graphics::image(x=sort(unique(x$G)), y=sort(unique(x$R)), z=a ,col = gradient ,xlab="Number of grouping units",ylab="Replicates per grouping unit", xlim=c(min(x$G),max(x$G)), ylim=c(min(x$R),max(x$R)), axes=FALSE)}
